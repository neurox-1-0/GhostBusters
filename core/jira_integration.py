"""Organization-scoped Jira configuration; credentials stay in environment secrets."""
...
class JiraIntegrationStore:
    ...
    def get(self, organization_id: UUID) -> JiraIntegrationConfig:
        with self._lock:
            current = self._configs.get(organization_id)
            if current is None:
                now = utc_now()
                current = JiraIntegrationConfig(organization_id=organization_id, created_at=now, updated_at=now)
                self._configs[organization_id] = current
            return current.model_copy(deep=True)

    def update(self, organization_id: UUID, request: JiraIntegrationConfigRequest) -> JiraIntegrationConfig:
        with self._lock:
            ...
            if "allowed_projects" in values:
                values["allowed_projects"] = sorted({str(item).strip().upper() for item in values["allowed_projects"] if str(item).strip()})
            updated = current.model_copy(update={**values, "updated_at": utc_now()})
            self._configs[organization_id] = updated
            self._persist()
            return updated.model_copy(deep=True)

    def mark_validation(self, organization_id: UUID, success: bool, failure: str | None = None) -> JiraIntegrationConfig:
        with self._lock:
            current = self.get(organization_id)
            updated = current.model_copy(update={"last_validated": utc_now(), "updated_at": utc_now(), "last_failure_summary": None if success else (failure or "Jira validation failed safely.")})
            self._configs[organization_id] = updated
            self._persist()
            return updated.model_copy(deep=True)

    def mark_collection(self, organization_id: UUID, success: bool, failure: str | None = None) -> JiraIntegrationConfig:
        with self._lock:
            current = self.get(organization_id)
            updated = current.model_copy(update={"last_successful_collection": utc_now() if success else current.last_successful_collection, "updated_at": utc_now(), "last_failure_summary": None if success else (failure or "Jira collection failed safely.")})
            self._configs[organization_id] = updated
            self._persist()
            return updated.model_copy(deep=True)
    ...
    def _load(self) -> None:
        ...
        for key, value in payload.items():
            try: self._configs[UUID(key)] = JiraIntegrationConfig.model_validate(value)
            except Exception: continue
...
jira_integration_store = JiraIntegrationStore()
