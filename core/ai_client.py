...
class GeminiAIClient:
    ...
    def validate_goal(self, payload: dict[str, Any]) -> AICallResult:
        prompt = json.dumps({"task": "validate_cloud_cost_goal", "instructions": "Return only the schema. Prefer zero clarification questions. Ask only when an answer cannot be obtained from authorized tools or supplied context and is essential to safety, authorized scope, or the success definition; otherwise accept with concise, explicit safe assumptions. Use supplied integration context and defaults before asking. Do not ask the user for current spend when connected billing or cost evidence can be collected, and do not ask about optimization initiatives unless they materially constrain an already-scoped plan. Price-list data may estimate unit prices but never proves actual spend. Treat read-only investigation, no direct mutation, production protection, and human approval as defaults, not questions. Ask at most three questions, prefer safe choice options, and allow at most one recommended single-choice option. Assess relevance and ambiguity; do not authorize mutation, invent facts, tools, permissions, costs, or evidence.", "input": redact_model_payload(payload)}, sort_keys=True)
