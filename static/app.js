...
function renderGoalTab() {
  ...
  if (state.goalTab === "plan") { const planned = run.original_plan?.selected_capabilities || []; const selected = planned.length ? planned : (run.selected_tools || []).map((capability) => ({ capability, reason: "Selected by the safe execution path.", expected_evidence: "Verified evidence or an explicit gap." })); append(node, el("h3", "card-title", run.planning_mode === "gemini_primary" ? "Gemini-assisted plan" : "Safe execution plan"), el("p", "muted", run.original_plan?.decision_summary || "Capabilities are constrained to read-only evidence collection and safety checks.")); selected.forEach((item, index) => { const card = el("article", "goal-finding-card"); append(card, el("span", "status-badge status-in-progress", `Step ${index + 1}`), el("h3", null, labelFor(item.capability || item)), el("p", null, item.reason || "Selected by the safe execution path."), dataList([["Expected evidence", item.expected_evidence || "Verified evidence or an explicit gap."], ["Status", index < goalStageIndex(run) ? "Completed" : index === goalStageIndex(run) ? "Current" : "Not started"]])); node.appendChild(card); }); if (run.plan_revisions?.length) { node.appendChild(el("h3", "card-title", "Plan revisions")); run.plan_revisions.forEach((revision) => node.appendChild(el("p", "goal-next-action", revision.reason || "Plan revised from new evidence."))); } return; }
