...
function renderGoalTab() {
  ...
  if (state.goalTab === "outcome") {
    ...
    const assessment = [...(run.plan_revisions || [])].reverse().find((item) => item.kind === "agent_assessment");
    const assessmentCard = assessment ? el("article", "goal-finding-card goal-agent-assessment") : null;
    if (assessmentCard) append(assessmentCard, el("span", "status-badge status-in-progress", "Agent assessment"), el("h3", null, assessment.headline || "Recorded evidence assessment"), el("p", null, assessment.summary || "No assessment was recorded."), dataList([["Provider", assessment.provider || "Not recorded"], ["Evidence used", (assessment.evidence_points || []).join(" ") || "No evidence summary recorded"], ["Limitations", (assessment.uncertainty_points || []).join(" ") || "None recorded"], ["Next step", assessment.next_step || "Review the recorded outcome."]]));
    append(node, hero, metrics, assessmentCard, el("h3", "card-title", "Recommended next action"), el("p", "goal-next-action", run.status === "needs_more_evidence" ? missingAction : run.status === "pending_human_review" ? "Review the recommendation and approve, reject, or request more evidence." : "Review the recorded evidence and safety checks."), run.missing_evidence?.length ? dataList(run.missing_evidence.map((item) => ["Missing evidence", item])) : null, goalDestinationActions(run)); return;
  }
  ...
  if (state.goalTab === "plan") {
    ...
    const assessment = [...(run.plan_revisions || [])].reverse().find((item) => item.kind === "agent_assessment");
    if (assessment) {
      const card = el("article", "goal-finding-card goal-agent-assessment");
      append(card, el("span", "status-badge status-in-progress", "Agent assessment"), el("h3", null, assessment.headline || "Evidence conclusion"), el("p", null, assessment.summary || "No assessment was recorded."), dataList([["Provider", assessment.provider || "Not recorded"], ["Next step", assessment.next_step || "Review the recorded outcome."]]));
      node.appendChild(card);
    }
